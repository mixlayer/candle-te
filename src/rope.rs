...
fn tensor_device_ptr<T: CudaDType>(t: &Tensor, stream: &cudarc::driver::CudaStream) -> Result<u64> {
    let (s, l) = t.storage_and_layout();
    match &*s {
        candle::Storage::Cuda(cuda_storage) => {
            //TODO this _sync is almost certainly being dropped too early either causing
            // unsafety or perf degradation
            let (ptr, _sync) = cuda_storage.as_cuda_slice::<T>()?.device_ptr(&stream);
            Ok(ptr)
        }
...
impl NVTETensor {
    pub fn from_tensor<T: CudaDType + DeviceRepr>(tensor: &Tensor) -> Result<Self> {
        let (storage, layout) = tensor.storage_and_layout();
        match &*storage {
            Storage::Cuda(cuda_storage) => Self::from_storage::<T>(cuda_storage, &layout),
            _ => panic!("nvte only supports cuda"),
        }
    }
...
impl NVTEFusedRope {
    ...
    fn cuda_fwd_t<T: CudaDType + DeviceRepr>(
        ...
        // let cu_seqlens_ptr = tensor_device_ptr::<u32>(&self.cu_seqlens, &stream)?;
        // let freqs_ptr = tensor_device_ptr::<f32>(&self.freqs, &stream)?;
        // let start_positions_ptr = tensor_device_ptr::<u32>(&self.start_positions, &stream)?;

        let cu_seqlens_nvte_tensor = NVTETensor::from_tensor::<u32>(&self.cu_seqlens)?;
        let start_positions_nvte_tensor = NVTETensor::from_tensor::<u32>(&self.start_positions)?;
        let freqs_nvte_tensor = NVTETensor::from_tensor::<f32>(&self.freqs)?;
        ...
        let b_sz = self.cu_seqlens.shape().dims1()?;
